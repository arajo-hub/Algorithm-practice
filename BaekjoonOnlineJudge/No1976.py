...
N = int(sys.stdin.readline()) # 노드 수 입력 받기
M = int(sys.stdin.readline()) # 정점 수 입력 받기
...
for y in range(1, N+1): # y번째 도시
    maps = list(map(int, sys.stdin.readline().split())) # y번째 도시가 어느 도시와 연결되어 있는지 정보
    for x in range(1, len(maps)+1): # y도시 연결 정보 추출
        if maps[x-1] == 1: # x 도시와 연결되어 있으면
            union(y, x) # 두 도시를 결합

tour = list(map(int, sys.stdin.readline().split())) # 여행 계획 정보
...
# 출처 : https://deep-learning-study.tistory.com/590