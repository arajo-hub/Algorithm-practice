...
def solution(n, weak, dist): # n은 외벽의 길이, weak는 취약지점, dist는 각 친구가 1시간에 이동할 수 있는 거리
    length=len(weak) # 취약지점이 몇 개인지 보고
    for i in range(length):
        weak.append(weak[i]+n) # 레스토랑이 원형형태를 취하고 있다고 했으므로 길이를 2배로 늘려준다.
    answer=len(dist)+1
    for start in range(length): # 취약지점을 하나씩 뽑아서
        for friends in list(permutations(dist, len(dist))): # permutations는 모든 친구들을 줄세우는 경우의 수(순열)
            count=1 # 투입할 친구의 수
            position=weak[start]+friends[count-1] # 해당 친구가 점검할 수 있는 마지막 위치
            for index in range(start, start+length): # 시작지점부터 모든 취약지점을 확인
                if position<weak[index]: # 해당 친구가 점검할 수 없는 위치라면
                    count+=1 # 새로운 친구를 넣는다.
                    if count>len(dist):
                        break
