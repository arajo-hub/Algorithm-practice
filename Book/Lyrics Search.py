...
def countByRange(array, start, end): # 배열, 시작값, 끝값을 이용해서, 시작값과 끝값의 인덱스를 찾고 그 인덱스 차로 개수를 구한다.
    startIndex=bisect_left(array, start)
    endIndex=bisect_right(array, end)
    return endIndex-startIndex

def solution(words, queries):
    answer=[]
    for word in words:
        array[len(word)].append(word) # 같은 길이별로 문자를 나눈다.
        reversed_array[len(word)].append(word[::-1]) # 거꾸로 뒤집은 문자도 별도의 배열에 넣는다.
    
    for i in range(10001): # 배열의 각 칸에 들어있는 단어들을 정렬한다.
        array[i].sort()
        reversed_array[i].sort()
    
    for q in queries:
        if q[0]!='?': # 검색하는 단어의 접미사가 와일드카드라면
            res=countByRange(array[len(q)], q.replace('?', 'a'), q.replace('?', 'z'))
        else:
            res=countByRange(reversed_array[len(q)], q[::-1].replace('?', 'a'), q[::-1].replace('?','z'))
        answer.append(res)
    return answer
