...
dp=[10001]*(m+1)
dp[0]=0
for i in range(n): # 화폐 개수만큼 for문을 돌린다.
    for j in range(money[i], m+1): # 입력된 화폐~m원만큼 for문을 돌린다.
        if dp[j-money[i]]!=10001:
            dp[j]=min(dp[j], dp[j-money[i]+1]) # j원을 만드는 경우의 수는 1. j원을 만드는 경우의 수와
                                                # 2. j에서 화폐단위를 하나 뺀 거에서 +1(화폐단위 하나 더 하면 만들 수 있으므로)의 값 중
                                                # 작은 값으로 한다.
